jQuery(document).ready(function ($) {
  ...
  function updateFilling(selectedEvent, filling, totWidth) {
    ...
    eventLeft =
      Number(eventLeft.replace('px', '')) + Number(eventWidth.replace('px', '')) / 2
  ...
  function updateVisibleContent(event, eventsContent) {
    ...
    if (eventDate) {
      const monthNo = parseInt(eventDate.split('/')[0]) + 1
      let prec = 4
      prec = monthNo / 24

      window.viewer.clippingBox.scale.set(100000000, 100000, prec * 8)

      const $circularProgress = $('.circular-progress'),
        $progressValue = $('.progress-value')

      let progressStartValue = 0,
        progressEndValue = Math.round(prec * 100),
        speed = 5
